...
import { buildSlotViews } from "@/lib/schedule-view";
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Assembly lives in lib/schedule-view so the daily email describes exactly
    // the same day this does — see the note at the top of that file.
    const posts = await buildSlotViews(start, end);
